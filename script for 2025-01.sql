...
-- 1) table contact_for  
create table `contact_for_202501_lcc` (
	  ...
	  constraint `contact_for_202501_lcc_ibfk_1` foreign key (`file_id`) references `file_details` (`id`)
...
insert into contact_for_202501_lcc
...
select count(*)  from contact_for_202501_lcc;  --  number need to delete becaues contracted or invalid number

delete from contact_for_202501_lcc
...
select branch_name , count(*)  from contact_for_202501_lcc group by branch_name ;

update contact_for_202501_lcc set branch_name = 'Bokeo' where branch_name is null;

select count(*) from contact_for_202501_lcc cfl -- 
...
select '2024-12-31' `date`, `contact_no`, `contact_id`, `remark_2`, `status_updated`
from contact_for_202410_lcc cfl where status_updated is not null;


-- 7) check and update logcall 
select cfl.contact_id, t.`count_time` from contact_for_202501_lcc cfl
...
-- update
update contact_for_202501_lcc cfl
left join (select contact_id, count(*) `count_time` from contact_for_logcall group by contact_id) t on (cfl.contact_id = t.contact_id)
set cfl.`condition` = t.`count_time` ;

-- update the condition is null
select * from contact_for_202501_lcc where `condition` is null; -- 1,356,166
update contact_for_202501_lcc set `condition` = 1 where `condition` is null and status is not null; -- 1,335,744
update contact_for_202501_lcc set `condition` = 1 where `condition` is null and status is null; -- 422


-- run for only Nov 2024
update contact_for_202501_lcc cntl left join file_details fd on (fd.id = cntl.file_id)
set `remark_1` =
...
# ______________________________________________________ export to create campaign on LCC for contact_for_202501_lcc ______________________________________________________________ #
...
from contact_for_202501_lcc cntl 

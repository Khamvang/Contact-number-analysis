...
-- 1) table contact_for  
create table `contact_for_202404_lcc` (
	  ...
	  constraint `contact_for_202404_lcc_ibfk_1` foreign key (`file_id`) references `file_details` (`id`)
) engine=InnoDB auto_increment=1 default CHARSET=utf8mb4 collate utf8mb4_general_ci ;


-- 2) intert data from contact_numbers_to_lcc to table monthly
insert into contact_for_202404_lcc
...
select count(*)  from contact_for_202404_lcc;  --  number need to delete becaues contracted or invalid number

delete from contact_for_202404_lcc
...
select branch_name , count(*)  from contact_for_202404_lcc group by branch_name ;

update contact_for_202404_lcc set branch_name = 'Bokeo' where branch_name is null;

select count(*) from contact_for_202404_lcc cfl -- 
...
select cfl.contact_id, t.`count_time` from contact_for_202404_lcc cfl
...
-- update
update contact_for_202404_lcc cfl
left join (select contact_id, count(*) `count_time` from contact_for_logcall group by contact_id) t on (cfl.contact_id = t.contact_id)
set cfl.`condition` = t.`count_time` ;

-- update the condition is null
select * from contact_for_202404_lcc where `condition` is null; -- 1,356,166
update contact_for_202404_lcc set `condition` = 1 where `condition` is null and status is not null; -- 1,335,744
update contact_for_202404_lcc set `condition` = 1 where `condition` is null and status is null; -- 422




# ______________________________________________________ export to create campaign on LCC for contact_for_202404_lcc ______________________________________________________________ #

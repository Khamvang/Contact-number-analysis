...
TRUNCATE TABLE `contact_for_202508_lcc`;


-- 1) table contact_for  
create table `contact_for_202508_lcc` (
	...
	constraint `contact_for_202508_lcc_ibfk_1` foreign key (`file_id`) references `file_details` (`id`)
...
insert into contact_for_202508_lcc
...
select count(*)  from contact_for_202508_lcc;  --  number need to delete becaues contracted or invalid number

delete from contact_for_202508_lcc
...
select branch_name , count(*)  from contact_for_202508_lcc group by branch_name ;

update contact_for_202508_lcc set branch_name = 'Bokeo' where branch_name is null or branch_name = '';

select count(*) from contact_for_202508_lcc cfl -- 
...
select cfl.contact_id, t.`count_time` from contact_for_202508_lcc cfl
...
-- update
update contact_for_202508_lcc cfl
left join (select contact_id, count(*) `count_time` from contact_for_logcall group by contact_id) t on (cfl.contact_id = t.contact_id)
set cfl.`condition` = t.`count_time` ;

-- update the condition is null
select * from contact_for_202508_lcc where `condition` is null; -- 1,356,166
update contact_for_202508_lcc set `condition` = 1 where `condition` is null and status is not null; -- 1,335,744
update contact_for_202508_lcc set `condition` = 1 where `condition` is null and status is null; -- 422



select * from contact_for_202508_lcc order by id desc 51015810;

ALTER TABLE contact_for_202508_lcc AUTO_INCREMENT = 60000000;
...
update contact_for_202508_lcc set contact_no = 
...
update contact_for_202508_lcc cntl left join file_details fd on (fd.id = cntl.file_id)
set `remark_1` =
...
select * from contact_for_202508_lcc cntl where cntl.branch_name = '' or cntl.branch_name is null



# ______________________________________________________ export to create campaign on LCC for contact_for_202508_lcc ______________________________________________________________ #
...
FROM contact_for_202508_lcc cntl 
WHERE 

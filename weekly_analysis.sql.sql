...
select status, status_updated, count(*)  from contact_for_202410_lcc cntl where cntl.id in (select id from temp_update_any)  group by status, status_updated ;

-- 8)update status in table contact_for_202410_lcc 
update contact_for_202410_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
...
select `id`, `contact_no`, `remark_3`, `status`, 0 `pbxcdr_time`, `contact_id`  from contact_for_202410_lcc cfl where status_updated = 'NO ANSWER'
...
update contact_for_202410_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
set cntl.remark_2 = tua.remark_3, cntl.status_updated = tua.status
-- select cntl.remark_2 , tua.remark_3, cntl.status_updated , tua.status from contact_for_202410_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 

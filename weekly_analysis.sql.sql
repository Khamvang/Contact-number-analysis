...
WHERE LENGTH(contact_no) IN (11,12) and `date_created` between '2024-04-29' and '2024-05-28'; -- copy last date_created to here
...
where length(contact_no) in (11,12) and `date_created` between '2024-04-29' and '2024-05-28'; 
...
where 1=1 and c.`rank` is not null and c.`rank` != '' and campaigns.created_at between '2024-04-29' and '2024-05-28' order by cc.created_at desc;
...
-- 8)update status in table contact_for_202405_lcc 
update contact_for_202405_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
...
select `id`, `contact_no`, `remark_3`, `status`, 0 `pbxcdr_time`, `contact_id`  from contact_for_202405_lcc cfl where status_updated = 'NO ANSWER'
...
update contact_for_202405_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
set cntl.remark_2 = tua.remark_3, cntl.status_updated = tua.status
-- select cntl.remark_2 , tua.remark_3, cntl.status_updated , tua.status from contact_for_202405_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 

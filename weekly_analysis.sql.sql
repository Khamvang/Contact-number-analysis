...
WHERE LENGTH(contact_no) IN (11,12) and `date_created` between '2024-02-29' and '2024-03-31'; -- copy last date_created to here
...
where length(contact_no) in (11,12) and `date_created` between '2024-02-29' and '2024-03-31'; 
...
where  status = 'ANSWERED' and communication_type = 'Outbound'
 ...
 and date_format(`time`, '%Y-%m-%d') between '2024-02-29' and '2024-03-31' -- please chcek this date from table all_unique_analysis
...
where 1=1 and c.`rank` is not null and c.`rank` != '' and campaigns.created_at between '2024-02-29' and '2024-03-31' order by cc.created_at desc;

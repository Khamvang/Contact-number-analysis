...
WHERE LENGTH(contact_no) IN (11,12) and `date_created` between '2025-04-30' and '2025-05-28'; -- copy last date_created to here
...
where length(contact_no) in (11,12) and `date_created` between '2025-04-30' and '2025-05-28'; 
...
where -- status = 'ANSWERED' and communication_type in ('Outbound', 'Internal')
 ...
 and date_format(`time`, '%Y-%m-%d') between '2025-04-30' and '2025-05-28' -- please chcek this date from table all_unique_analysis
...
where 1=1 and c.`rank` is not null and c.`rank` != '' and campaigns.created_at between '2025-04-30' and '2025-05-28' order by cc.created_at desc;
...
-- 8)update status in table contact_for_202505_lcc 
update contact_for_202505_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
...
update contact_for_202505_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 
set cntl.remark_2 = tua.remark_3, cntl.status_updated = tua.status
-- select cntl.remark_2 , tua.remark_3, cntl.status_updated , tua.status from contact_for_202505_lcc cntl left join temp_update_any tua on (cntl.contact_id = tua.contact_id) 

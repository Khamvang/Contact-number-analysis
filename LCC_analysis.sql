...
WHERE LENGTH(contact_no) IN (11,12) and `date_created` between '2025-07-30' and '2025-08-30'; -- copy last date_created to here
...
where length(contact_no) in (11,12) and `date_created` between '2025-07-30' and '2025-08-30'; 
...
where -- status = 'ANSWERED' and communication_type in ('Outbound', 'Internal')
 ...
 and date_format(`time`, '%Y-%m-%d') between '2025-07-30' and '2025-08-30' -- please chcek this date from table all_unique_analysis
...
where 1=1 and c.`rank` is not null and c.`rank` != '' and campaigns.created_at between '2025-07-30' and '2025-08-30' order by cc.created_at desc;
...
select 
	...
	status_updated, 
	count(*) 
from contact_for_202507_lcc cfl 
group by 
	remark_2,
	status_updated
;
